...
static int irq_setup(unsigned int *irqs, unsigned int nvec, unsigned int start_numa_node)
{
	...
	if (!alloc_cpumask_var(&curr, GFP_KERNEL)) {
		err = -ENOMEM;
		goto free_curr;
	}
	...
	rcu_read_lock();
	for_each_numa_hop_mask(next, start_numa_node) {
